...
*/
char *aesd_circular_buffer_add_entry(struct aesd_circular_buffer *buffer, const struct aesd_buffer_entry *add_entry, size_t *ret_entry_size)
{
    char *res = NULL;
    ...
    if(buffer->full)
    {
        ...
        *ret_entry_size = buffer->entry[buffer->out_offs].size;
        buffer->out_offs++;
